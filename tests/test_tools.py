...
class TestDiagnosticAllowlist:
    ...
    # Workstream H: new diagnostic tools
    def test_dns_dig(self):
        assert _is_diagnostic_allowed("dig example.com") is True
        assert _is_diagnostic_allowed("dig +short example.com A") is True

    def test_dns_nslookup_with_arg(self):
        assert _is_diagnostic_allowed("nslookup example.com") is True

    def test_dns_nslookup_bare_blocked(self):
        assert _is_diagnostic_allowed("nslookup") is False

    def test_dns_host(self):
        assert _is_diagnostic_allowed("host") is True

    def test_cert_openssl(self):
        assert _is_diagnostic_allowed("openssl s_client -connect example.com:443") is True

    def test_ping_bounded(self):
        assert _is_diagnostic_allowed("ping -c 4 example.com") is True

    def test_ping_unbounded_blocked(self):
        assert _is_diagnostic_allowed("ping example.com") is False

    def test_traceroute(self):
        assert _is_diagnostic_allowed("traceroute example.com") is True

    def test_ssrf_curl_external_still_blocked(self):
        assert _is_diagnostic_allowed("curl -sI http://169.254.169.254/latest/meta-data/") is False
        assert _is_diagnostic_allowed("curl -sI http://evil.com") is False
...
class TestToolDefinitions:
    ...
    def test_definitions_is_nonempty_list(self):
        ...
        # 7 investigation tools + submit_report (Workstream A2)
