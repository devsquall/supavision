...
### Admin tooling
- **`supavision backup` CLI.** Operators previously had to remember `sqlite3 .supavision/supavision.db ".backup file.db"` to snapshot the DB. New one-shot command wraps the online `.backup` API (safe while serve / scheduler are running), defaults the destination to `<db>.backup-<UTC-timestamp>`, refuses to overwrite an existing file unless `--force` is set, and creates parent dirs as needed.
...
- **Admin actions written to audit log.** API key create/revoke and resource create/delete (from both the dashboard AND the CLI) now write entries to `auth_audit_log` (event types: `api_key_created`, `api_key_revoked`, `resource_created`, `resource_deleted`). CLI-originated events carry `source=cli` in the detail string to distinguish them from dashboard-originated events. Previously these had no audit trail — only authentication flows were logged. The new event types are also in the `/settings/audit-log` filter dropdown.
