...
- **Admin actions written to audit log.** API key create/revoke and resource create/delete now write entries to `auth_audit_log` (event types: `api_key_created`, `api_key_revoked`, `resource_created`, `resource_deleted`). Previously these had no audit trail — only authentication flows were logged. The new event types are also in the `/settings/audit-log` filter dropdown.
