...
def _sparkline_points(values: list[int], width: int = 64, height: int = 18) -> str:
...
def _severity_streak(store, resource_id: str, current_severity: str, max_lookback: int = 10) -> int:
    """Count consecutive recent evaluations with the same severity (Workstream B).

    Returns 1 if only the latest evaluation matches (no streak to report), or N
    if the last N evaluations all had this severity. Capped at `max_lookback` to
    avoid scanning the entire history.
    """
    try:
    ...
    count = 0
    for ev in evals:
        if str(ev.severity) == current_severity:
            count += 1
        else:
            break
    return max(count, 1)
