"""Scheduler — runs discovery, health checks, and codebase scans on schedule.
...
Routes by resource_type:
  - Infrastructure (server, aws_account, database, github_org):
      discovery_schedule → Engine.run_discovery()
      health_check_schedule → Engine.run_health_check()
  - Codebase:
      discovery_schedule → CodebaseEngine.run_scan()
      (health_check_schedule is unused for codebases)
"""
...
class Scheduler:
    ...
    def _get_due_jobs(self) -> list[tuple[Resource, RunType]]:
        ...
        for resource in resources:
            ...
            # Codebase resources use discovery_schedule for scans (RunType.SCAN)
            if resource.resource_type == "codebase":
                schedule_pairs = [
                    (RunType.SCAN, resource.discovery_schedule),
                ]
            else:
                schedule_pairs = [
    ...
    def _execute_run(self, resource: Resource, run_type: RunType) -> None:
        ...
        try:
            if run_type == RunType.SCAN:
                from .codebase_engine import CodebaseEngine
                codebase_engine = CodebaseEngine(self.store)
                codebase_engine.run_scan(resource.id)
    ...
    async def _execute_run_async(self, resource: Resource, run_type: RunType) -> None:
        ...
        try:
            if run_type == RunType.SCAN:
                ...
                await loop.run_in_executor(None, codebase_engine.run_scan, resource.id)
            elif self.engine is None:
                logger.warning(
                    "Skipping %s for %s: infrastructure engine not available (install Claude CLI)",
                    run_type, resource.name,
