...
"""
    ...
    # ── 2. Codebase resource with real scanner findings ─────────────

    # Find a suitable project to scan — use the supervisor repo itself
    scan_path = str(Path(__file__).resolve().parent.parent.parent.parent)
    if not Path(scan_path).is_dir() or not (Path(scan_path) / "src").is_dir():
        scan_path = os.getcwd()

    codebase = Resource(
        name="api-backend",
        resource_type="codebase",
        config={"path": scan_path, "language_hint": "python"},
        discovery_schedule=Schedule(cron="0 0 * * 0"),
    )
    store.save_resource(codebase)
    ...
    try:
        run = engine.run_scan(codebase.id)
        items, total = store.list_work_items(resource_id=codebase.id)
        print(f"  Created codebase resource: {codebase.name} ({total} findings)", file=sys.stderr)

        # Transition some findings to show the workflow
        evaluated = 0
        approved = 0
        for item in items:
            if evaluated < 2 and item.stage == FindingStage.SCANNED:
                item.evaluation_verdict = "true_positive"
                item.evaluation_reasoning = (
                    "This pattern uses user-controlled input in a potentially "
                    "dangerous operation. The input flows from an external source "
                    "without proper sanitization."
                )
                item.evaluation_fix_approach = (
                    "Use parameterized queries or input validation to prevent injection."
                )
                item.evaluation_effort = "small"
                item.transition_to(FindingStage.EVALUATED)
                store.save_work_item(item)
                evaluated += 1
            elif approved < 1 and evaluated >= 2 and item.stage == FindingStage.SCANNED:
                item.evaluation_verdict = "true_positive"
                item.evaluation_reasoning = "Confirmed exploitable via user input."
                item.evaluation_fix_approach = "Sanitize input before use."
                item.evaluation_effort = "trivial"
                item.transition_to(FindingStage.EVALUATED)
                item.transition_to(FindingStage.APPROVED)
                store.save_work_item(item)
                approved += 1

    except Exception as e:
        print(f"  Warning: Scanner failed ({e}), codebase resource created without findings", file=sys.stderr)

    # ── 3. Seed notifications (for Alerts page) ─────────────────────
    notification_data = [
        ...
        (codebase.id, "slack", "info", "2 new findings detected in scan.", "sent", "", 2),
    ]
    for res_id, channel, severity, summary, status, error, days_ago in notification_data:
        ...
        store._commit()
    print("  Created 6 notification log entries", file=sys.stderr)

    # ── 4. Seed metrics (for Metrics page) ─────────────────────────
    import random
    ...
    # ── 5. Seed auth events (for Activity page) ────────────────────
    ...
    print(f"\nDone! {len(resources)} resources, {sum(stage_counts.values())} findings.", file=sys.stderr)
    ...
    _json_out({
        ...
        "findings": sum(stage_counts.values()),
    })
...
def cmd_mcp_config(args: argparse.Namespace) -> None:
    ...
    db_path = os.path.abspath(getattr(args, "db", None) or os.environ.get("SUPAVISION_DB_PATH", ".supavision/supavision.db"))
    config = {
        "mcpServers": {
            "supavision": {
...
def cmd_scan(args: argparse.Namespace) -> None:
    ...
    store = _get_store(args)
    engine = CodebaseEngine(store)
    run = engine.run_scan(args.resource_id)
    items, total = store.list_work_items(resource_id=args.resource_id, run_id=run.id)
    _json_out({
        "ok": True,
        "run_id": run.id,
...
def cmd_findings(args: argparse.Namespace) -> None:
    ...
    items, total = store.list_work_items(
        ...
        per_page=getattr(args, "limit", 50),
    )
    _json_out({
        "ok": True,
        "total": total,
...
def cmd_evaluate(args: argparse.Namespace) -> None:
    """Create an evaluation job for a work item."""
    from .codebase_engine import CodebaseEngine

    store = _get_store(args)
    item = store.get_work_item(args.work_item_id)
    if not item:
        _error(f"Work item {args.work_item_id} not found")
    engine = CodebaseEngine(store)
    job = engine.create_evaluate_job(args.work_item_id, item.resource_id)
    _json_out({
        "ok": True,
        "job_id": job.id,
        "job_type": job.job_type,
        "status": job.status.value,
        "message": "Job created. Start the agent runner to execute.",
    })
...
def cmd_implement(args: argparse.Namespace) -> None:
    ...
    if not item:
        _error(f"Work item {args.work_item_id} not found")
    engine = CodebaseEngine(store)
    job = engine.create_implement_job(args.work_item_id, item.resource_id)
    _json_out({
        "ok": True,
        "job_id": job.id,
        "job_type": job.job_type,
...
def cmd_scout(args: argparse.Namespace) -> None:
    """Launch a scout agent on a codebase resource."""
    from .codebase_engine import CodebaseEngine

    store = _get_store(args)
    engine = CodebaseEngine(store)
    focus = getattr(args, "focus", "general")
    job = engine.create_scout_job(args.resource_id, focus)
    _json_out({
        "ok": True,
        "job_id": job.id,
        "job_type": job.job_type,
...
def cmd_blocklist_list(args: argparse.Namespace) -> None:
...
def main() -> None:
    ...
    p.add_argument("resource_id", help="Resource ID")
    p.add_argument("--discovery", default="", help="Cron expression for discovery runs (e.g., '0 */6 * * *' for every 6 hours)")
    p.add_argument("--health-check", default="", help="Cron expression for health checks (e.g., '0 */6 * * *' for every 6 hours)")
    ...
    p.add_argument("resource_id", help="Resource ID")
    p.add_argument("--severity", choices=["healthy", "warning", "critical"], default="warning", help="Severity level for the test notification")
    ...
    # scan
    p = sub.add_parser("scan", help="Run regex scan on a codebase resource")
    p.add_argument("resource_id", help="Resource ID")
    p.set_defaults(func=cmd_scan)

    # findings
    p = sub.add_parser("findings", help="List findings for a resource")
    p.add_argument("resource_id", help="Resource ID")
    p.add_argument("--stage", default=None, help="Filter by stage: scanned, evaluated, approved, implementing, completed, rejected, dismissed")
    p.add_argument("--severity", default=None, help="Filter by severity: critical, high, medium, low, info")
    p.add_argument("--limit", type=int, default=50, help="Maximum number of findings to return")
    p.add_argument("--page", type=int, default=1, help="Page number for paginated results")
    p.set_defaults(func=cmd_findings)

    # evaluate
    p = sub.add_parser("evaluate", help="Create evaluation job for a work item")
    p.add_argument("work_item_id", help="Work item ID")
    p.set_defaults(func=cmd_evaluate)

    # implement
    p = sub.add_parser("implement", help="Create implementation job for a work item")
    p.add_argument("work_item_id", help="Work item ID")
    p.set_defaults(func=cmd_implement)

    # scout
    p = sub.add_parser("scout", help="Launch scout agent on a codebase resource")
    p.add_argument("resource_id", help="Resource ID")
    p.add_argument("--focus", default="general", choices=["general", "security", "performance", "quality"], help="Area of focus for the scout agent")
    p.set_defaults(func=cmd_scout)

    # approve
    p = sub.add_parser("approve", help="Approve a work item for implementation")
    p.add_argument("work_item_id", help="Work item ID")
    p.set_defaults(func=cmd_approve)

    # reject
    p = sub.add_parser("reject", help="Reject a work item")
    p.add_argument("work_item_id", help="Work item ID")
    p.set_defaults(func=cmd_reject)

    # blocklist
    sub.add_parser("blocklist", help="List blocklist entries").set_defaults(func=cmd_blocklist_list)
